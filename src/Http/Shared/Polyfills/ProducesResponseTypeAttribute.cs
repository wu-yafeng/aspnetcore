...
namespace Microsoft.AspNetCore.Mvc
{
    ...
    /// </summary>
    internal class ProducesResponseTypeAttribute : IApiResponseMetadataProvider
    {
        ...
        private static MediaTypeCollection GetContentTypes(string contentType, string[] additionalContentTypes)
        {
            ...
            foreach (var type in completeContentTypes)
            {
                if (type.Contains("*"))
                {
                    throw new InvalidOperationException("Wildcards are not supported in content types.");
                }
