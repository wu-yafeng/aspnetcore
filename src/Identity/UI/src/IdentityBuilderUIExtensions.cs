...
namespace Microsoft.AspNetCore.Identity
{
    ...
    public static class IdentityBuilderUIExtensions
    {
        ...
        internal class ViewVersionFeatureProvider : IApplicationFeatureProvider<ViewsFeature>
        {
            ...
            public ViewVersionFeatureProvider(bool isV4) => _isV4 = true;
            ...
            public void PopulateFeature(IEnumerable<ApplicationPart> parts, ViewsFeature feature)
            {
                ...
                foreach (var descriptor in feature.ViewDescriptors)
                {
                    if (IsIdentityUIView(descriptor))
                    {
                        if (_isV4)
                        {
                            descriptor.RelativePath = descriptor.RelativePath.Replace("V5","V4");
                        }
                        else
                        {
            ...
            private static bool IsIdentityUIView(CompiledViewDescriptor desc) => desc.RelativePath.StartsWith("/Identity", StringComparison.OrdinalIgnoreCase);
        }
    }
}
